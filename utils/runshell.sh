#!/bin/bash
# Runs a bash shell for a prebuilt docker image named 'myproject'
...
if [[ $GPU == *"NVIDIA"* ]]; then
    ...
    sudo docker run --gpus all -u $(id -u):$(id -g) -v $(pwd):/tf -it --rm -p 8889:8888 image-1 bash
else
    echo "Running with CPU only"
    sudo docker run -u $(id -u):$(id -g) -v $(pwd):/tf -it --rm -p 8889:8888 image-1 bash
fi
